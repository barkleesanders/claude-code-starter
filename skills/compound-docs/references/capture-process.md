...
**IF similar issue found**, present decision options:
...
**ELSE** (no similar issue found): Proceed directly to Step 4 (no user interaction needed).
...
Validate against schema: Load `schema.yaml` and classify the problem against the enum values defined in [yaml-schema.md](./yaml-schema.md). Ensure all required fields are present and match allowed values exactly.
...
**Determine category from problem_type:** Use the category mapping defined in [yaml-schema.md](./yaml-schema.md).
...
Then in the decision menu, add a note: "This might be worth adding to Required Reading (Option 2)." But **NEVER auto-promote**. User decides via decision menu (Option 2).
