...
### Worktree Safety Rule (MANDATORY — 2026-05-10 incident)

**Every worktree branch must be pushed to `origin` within ONE commit. Period.**

Why: trust pages worktree (3 ship-ready pages, 232 passing tests) was wiped before push reached origin. Branch was force-deleted, reflog gone, work unrecoverable. The single defense that would have saved it: `git push -u origin HEAD` after the first commit.

What's automated (you don't have to remember):
- **PostToolUse Bash hook** (`~/.claude/skills/hooks/post-bash-worktree-autopush.sh`): every `git commit` in a worktree triggers a background `git push -u origin HEAD` (30s timeout, logs to `~/.claude/logs/worktree-autopush.log`).
- **PreToolUse Bash hook** (`~/.claude/skills/hooks/pre-bash-worktree-protect.sh`): blocks `git worktree remove`, `git worktree prune`, `rm -rf .claude/worktrees/...`, `git branch -D worktree-*` when the target has uncommitted/unpushed work. Override only with `CLAUDE_ALLOW_WORKTREE_LOSS=1`.
- **Stop hook** (`~/.claude/skills/taskmaster/hooks/check-completion.sh`): refuses to let the session end if any worktree has unpushed commits.

What you must do:
1. Create worktrees via the `EnterWorktree` tool, not `git worktree add` directly.
2. Commit early, commit often. Each commit auto-pushes.
3. If `~/.claude/logs/worktree-autopush.log` shows a FAIL, fix it before continuing — don't ignore.
4. Before any cleanup: verify `git status` clean and `git log @{u}..HEAD` empty.
