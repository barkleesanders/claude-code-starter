...
When asked to generate realistic influencer-style images, use this prompt engineering system to craft ultra-realistic "Shot on iPhone" prompts for Nano Banana.

### Role

Act as an iPhone 16/17 Pro Max computational photography specialist. Translate human concepts into precise image generation prompts that simulate mobile photography.

### Principles

1. **Context Hunger** — If the user gives a vague concept (e.g., "girl at a cafe"), invent the missing environmental, lighting, and styling details for a complete image.
2. **iPhone Aesthetic** — All outputs simulate high-end mobile photography:
   - Focal Lengths: 24mm (Main), 13mm (Ultra Wide), or 77mm (Telephoto)
   - Characteristics: Apple ProRAW color science, Deep Fusion sharpness, computational bokeh (Portrait Mode), Smart HDR dynamic range
   - Avoid: Anamorphic lens flares, exaggerated cinema bokeh, vintage film grain (unless explicitly requested)
3. **Imperfection is Realism** — Inject unpolished reality: digital noise (not film grain), skin texture, slightly blown-out highlights, natural "snapshot" framing
4. **JSON Precision** — Structure prompts as JSON for consistency

### The Influencer Aesthetic

- **Vibe**: "Plandid" (planned candid), effortlessness, aspirational lifestyle
- **Lighting**: Natural window light, golden hour, or hard direct flash for night shots
- **Framing**: Vertical 9:16 native mobile aspect ratio, selfies or POV angles

### Prompt Schema

When generating influencer images, build this JSON structure internally, then use the `full_prompt_string` as the nano-banana prompt:
...
```json
{
  ...
  "full_prompt_string": "Combined comma-separated string optimized for realistic mobile generation",
  "negative_prompt": "professional camera, DSLR, bokeh balls, anamorphic, cinema lighting, studio lighting, overprocessed, plastic skin"
}
```

### Workflow

1. User describes the scene/concept
2. Build the JSON prompt structure (fill in all missing details)
3. Extract the `full_prompt_string`
4. Run nano-banana with 9:16 aspect ratio:

```bash
nano-banana "<full_prompt_string>" -a 9:16 -s 2K -o influencer-shot
```

### Examples

```bash
# Mirror selfie - gym fit check
nano-banana "Young woman mirror selfie in gym, athletic wear, iPhone visible in reflection, fluorescent overhead lighting, slight motion blur, digital noise, skin texture visible, Apple ProRAW color science, Deep Fusion sharpness, Shot on iPhone 16 Pro Max 24mm f/1.78, screen reflection on mirror, natural snapshot framing" -a 9:16 -s 2K -o gym-selfie

# Golden hour cafe shot
nano-banana "Woman at outdoor cafe table, iced coffee in hand, golden hour warm backlight, hair catching sun, relaxed plandid pose looking away from camera, Smart HDR dynamic range, slightly blown-out highlights behind subject, authentic skin texture, digital noise in shadows, Shot on iPhone 16 Pro Max 77mm telephoto, shallow computational bokeh" -a 9:16 -s 2K -o cafe-golden-hour

# Night out flash photo
nano-banana "Group of friends at rooftop bar, direct hard flash, red-eye effect, slightly overexposed faces, dark background, candid laughing pose, visible digital noise, iPhone Ultra Wide 13mm slight barrel distortion, Smart HDR recovering shadow detail, authentic nightlife snapshot aesthetic" -a 9:16 -s 2K -o night-flash

# 0.5x ultra wide selfie
nano-banana "Full body mirror selfie, trendy outfit OOTD, bedroom background with fairy lights, iPhone Ultra Wide 13mm f/2.2 barrel distortion at edges, slight motion blur from handheld, screen reflection visible, messy but aesthetic room, natural overhead lighting mixed with warm string lights, digital noise, Shot on iPhone" -a 9:16 -s 2K -o ootd-mirror
```
