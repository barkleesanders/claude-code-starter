...
### "No Fix Available" Does NOT Mean Unfixable (MANDATORY)

When Dependabot says `first_patched_version: null`, INVESTIGATE before accepting:

1. **Check for CDN/tarball from maintainer** — Some packages (e.g., SheetJS) pull from npm but publish fixes to their own CDN:
   ```json
   "xlsx": "https://cdn.sheetjs.com/xlsx-0.20.3/xlsx-0.20.3.tgz"
   ```
2. **Check for maintained forks** — Community forks often patch CVEs when maintainers abandon npm
3. **Check for alternative packages** — e.g., `exceljs` replaces `xlsx`, `undici` replaces `node-fetch`
4. **Force-commit lockfile** — Dependabot needs to see the resolved version in a committed lockfile to close the alert

**Example**: `xlsx` npm package frozen at 0.18.5 with 2 HIGH CVEs (CVE-2024-22363, CVE-2023-30533). Fix: `"xlsx": "https://cdn.sheetjs.com/xlsx-0.20.3/xlsx-0.20.3.tgz"` — both alerts auto-closed.

**Only WARN-and-continue** when you've verified no fix exists via npm, CDN, fork, OR alternative.
