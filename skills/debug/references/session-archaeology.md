...
| State | Path | Notes |
|-------|------|-------|
| Main config | `$HOME/.openclaw/openclaw.json` | ALWAYS backup before patching |
| Agent approval list | `$HOME/.openclaw/APPROVED_AGENTS` | One name per line |
| Agent sessions | `$HOME/.openclaw/agents/<id>/sessions/*.jsonl` | Event-sourced log (type=message/compaction/custom/etc) |
| Session checkpoints | same dir, `*.checkpoint.<uuid>.jsonl` | Snapshots at compaction boundaries |
| LCM database | `$HOME/.openclaw/lcm.db` | SQLite — only populated if `proactiveThresholdCompactionMode=inline` |
| Cron definitions | `$HOME/.openclaw/cron/jobs.json` | NOT in openclaw.json |
...
| Guardrail alerts | `$HOME/.openclaw/GUARDRAIL_ALERT` | Written by audit scripts — read tail for recent violations |
| Plugin source (inspect) | `$HOME/.openclaw/extensions/<plugin>/dist/*.js` | Grep compiled JS for enum values |
