...
## Pattern 24: Global App.tsx component vanishes after SPA→SSR conversion

**Symptom**: A widget that worked on every page — support/chat launcher, cookie banner, analytics, exit-intent, a `?param` deep-link handler — is silently gone on routes converted to SSR. No error, no console output, HTML renders fine.

**Root cause**: SSR does **not** mount the React tree. Anything mounted globally in `App.tsx` / the SPA root exists only inside that tree, so each converted route loses every global at once.

**Detection**: For each component mounted at the SPA root, confirm it is re-provided on the SSR page. Then test the *behavior*, not the markup: `?support=open` must **open the chat**, not merely return 200.

**Fix**: Re-provide each global as a shared island mounting the existing component, an SSR equivalent, or a working static fallback.

**Incident (2026-06-28, example)**: `?support=open` was dead on all 7 SSR pages. Fix `7a25f67`.

---

## Pattern 25: Bundled island/global CSS clobbers SSR inline design — invisible text

**Symptom**: Text is present in the DOM and readable in "view source", but **invisible on screen** — typically white/light headings on a light background. Zero errors, zero console output, a perfect HTML diff, passing CSP, and a passing `<h1>`-count check. Often ships **more than once** because every functional test passes.

**Root cause**: A bundled island/global stylesheet (commonly a Tailwind build that transitively imports the app's `index.css`) loads **after** the SSR page's inline `<style>` and overrides **element selectors** — `body{background:var(--color-bg);color:var(--color-text)}`. Same specificity, later in the cascade, so it wins. The SSR design is silently replaced.

**Fix**: Either the SSR layout's `body` background+color must win (`!important` — the leaked rules carry none), or the island CSS must not emit global `body` rules.

**Detection — computed style in a real browser, never a source grep.** This is the whole difficulty: nothing in the source or the HTML reveals it. Two ways to get it:
- **Pre-deploy (preferred)**: the CF integration harness + playwright — `~/.claude/skills/shared/cf-integration-harness.md` (this is /ship Phase 1.3d).
- **Post-deploy**: drive the live browser and read `getComputedStyle`.

### ⚠️ The contrast check MUST alpha-composite, or it lies

Implemented as *"first ancestor whose background isn't transparent"*, a contrast check produces confident **false positives**. Translucent tint panels (`rgba(255,190,46,.1)` gold, `rgba(255,255,255,.05)` white) over a dark body are common; treating a 10%-alpha tint as opaque compares gold text against gold and reports **1.00:1 "invisible text"** for a panel that is actually gold-on-navy and perfectly legible. This produced **6 bogus failures** across AIVA's SSR routes on first run (2026-07-27).

Collect every layer with `a > 0` up to the first **opaque** one, then composite back down:

```js
let base = layers[layers.length - 1];            // opaque base
for (let i = layers.length - 2; i >= 0; i--) {   // blend upward toward the element
  const t = layers[i];
  base = { r: t.r*t.a + base.r*(1-t.a), g: t.g*t.a + base.g*(1-t.a), b: t.b*t.a + base.b*(1-t.a), a: 1 };
}
```

Also skip non-painted nodes (`display:none`, `visibility:hidden`, zero-size rect), and use AA-large **3.0** for `>=24px` (or `>=18.66px` bold), **4.5** otherwise.

**Incident (2026-06-28, example)**: `support-island.css` — added to all 7 SSR pages by the Pattern 24 chat fix — leaked `body{background:#f0f0f0}`, making white headings invisible sitewide. Shipped **twice** undetected because the checks verified "chat opens", not "the page is readable". Fix `cd0cdfe` (`!important` on the layout's body rule).

**Verified still holding (2026-07-27)**: injecting a *non*-important `body{background:#f0f0f0}` into the built stylesheet leaves the harness green — the `!important` guard genuinely wins. Only an `!important` leak turns it red (13 failures). If you negative-test this pattern, escalate the injection accordingly, or a green run will read as a broken detector when it is actually a working fix.
