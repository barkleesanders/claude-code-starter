...
- Large bundle sizes
- **Hot-path full-partition scan** — a route running `COUNT(*)`/`SUM(CASE...)`/`ORDER BY x LIMIT 1 OFFSET N/2` over `WHERE <partition_key>=?` on every request (esp. *before* the cache check). Measure: `wrangler d1 execute <DB> --remote --json --command "..."` → `meta.rows_read`; >~100k = bug. Fix: cache it (slow-changing aggregate), fold into the cached payload, scope to the window, or drop it. See the Hot-Path Data-Volume & Cache-Topology Audit in `debug-patterns.md`.
- **Cache reader ↔ writer mismatch** — the route reads cache key `A`; the warmer cron warms key `B` (different shape, e.g. `oak311:open_data:...` vs `agg:...`), or nothing warms `A` at all. Permanent cache MISS → the route always pays the cold path. `grep` for the writer of every key the route reads, and for the reader of every key a `prewarm*`/cron writes.
- **Lagged-source `now()`-anchored window** — `WHERE ts >= now() - <interval>` against a source that publishes with a lag (Socrata/DataSF, batch ETL) is empty for narrow intervals; anchor on `MAX(ts)` instead.
- **TEXT-timestamp index defeated** — range scan where stored format (`'...000'`, floating) ≠ comparison literal (`'...123Z'`, ISO-with-`Z`); `EXPLAIN QUERY PLAN` must show `USING INDEX`.
...
- [ ] **Hono Set-Cookie**: every `c.header("Set-Cookie", ...)` call uses `{append: true}` — outer middleware silently clobbers inner cookies otherwise (real bug 2026-05-05: aiva_visit wiped by securityHeaders' aiva_cf for ~3h debugging). Same rule for `Vary`, `Link`, any multi-valued header.
- [ ] **Hono cookie middleware after ASSETS.fetch**: middlewares that wrap a SPA catch-all returning `c.env.ASSETS.fetch(c.req.raw)` MUST set Set-Cookie via `c.header(..., {append: true})` AFTER `await next()`. Pre-`next()` `setCookie(c, ...)` is dropped when the handler returns a Response object directly.
