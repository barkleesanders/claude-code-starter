...
### Device-frame / iframe previews — render at real device width, scale-to-fit (MANDATORY — 2026-06-17)

When embedding an app/site inside a phone (or any device) mockup frame, NEVER let the inner page render at the frame's narrow CSS width — a ~390px mobile layout in a ~240px iframe overflows and looks broken. Render the iframe at a REAL device width (390/393/430/768) and shrink with `transform: scale(innerW/deviceW); transform-origin: top left` on an `overflow:hidden` parent (iframe height = visibleH/scale). Add `scrolling="no"` to preview iframes; hide the app's scrollbar (`scrollbar-width:none`). Watch the `<span>`-styled-as-block-collapses-to-0×0 trap on notch/bezel elements. Verify in-browser: `iframe.contentWindow.innerWidth === deviceW` and no horizontal overflow. Full recipe: `~/.claude/skills/design/references/code-implementation.md` → "Device-Frame Previews". Reference incident: SUSU mockup — Dashboard/Activity/Loans overflowed their bezels.
