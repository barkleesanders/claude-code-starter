...
## Pre-1: Live Site fcdp Warmup (for web projects with a production URL)

Before investigating bugs or verifying fixes on deployed sites, warm up `fcdp` so you can take screenshots and inspect the live page — it drives the user's REAL Default-profile Chrome directly, no clone, no re-login:

```bash
FCDP=~/tools/fcdp/fcdp

# Find the production tab (adjust URL for the project)
TARGET=$($FCDP tabs 2>/dev/null | grep "aiva-m9t.pages.dev\|example.com" | awk '{print $1}' | head -1)

# If found, pre-warm by reading it once
if [ -n "$TARGET" ]; then
  $FCDP read "$TARGET" > /dev/null 2>&1 && echo "[fcdp] tab warm for $TARGET"
fi
```

Then use `$FCDP shot [tab] [file.png]` to screenshot, `$FCDP read [tab]` for the interactive-element outline, `$FCDP js [tab] "..."` to run JS — bridge down? `launchctl kickstart -k gui/$(id -u)/com.barklee.fcdp-bridge`.
...
# CRITICAL — EMBEDDED JS STRING SAFETY (after Biome auto-fix):
# Biome's noUselessEscapeInString rule does NOT understand strings-within-strings.
# In projects with inline HTML templates (CF Workers, SSR, template literals with <script>),
# Biome can silently break embedded JS by removing escapes like \' inside single-quoted strings.
# Example: 'Didn\'t' → 'Didn't' — the unescaped apostrophe terminates the string,
# killing the entire client-side script (no map, no data, no interactivity).
#
# After EVERY biome --fix on inline HTML projects:
if grep -q "text/html\|<script>" src/*.ts src/*.tsx 2>/dev/null; then
  echo "INLINE HTML PROJECT — checking for Biome string-escape damage..."
  # Look for apostrophes inside single-quoted JS strings in template literals
  grep -nP ":'[^']*'[^']*'" src/*.ts 2>/dev/null | grep -v "^.*//.*$" | head -20
  # After deploy: verify JS runs: curl the page and check for key function names
fi
