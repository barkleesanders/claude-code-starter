...
### AI Source Selection (Will ChatGPT / LLMs cite you?)

Classic SEO asks "can Google rank it?" AI assistants add a separate, mechanical question: **can an LLM's crawler FETCH the page, and once fetched, will it CITE you?** A site can be perfectly Google-indexed and totally ChatGPT-invisible.

**Run this FIRST on any AI-visibility audit** — a WAF/edge 403 to AI bots nullifies every other optimization, and `robots.txt: Allow /` does NOT override it (the block is a lower layer):
...
**One-command gate (run this — it encodes the two-layer check below):**

```bash
~/.claude/skills/seo-audit/scripts/ai-crawl-reachability.sh <domain> [/a-deep-route]
