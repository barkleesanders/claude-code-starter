...
Use the authenticated `gog` CLI for one-off uploads:

```bash
gog upload <local-file> -a <account> --parent <folder-id> --json --results-only
```

Use `rclone` for bulk sync or path-based mirrors. Never embed an API key, OAuth token, cookie, or credential-bearing URL in this skill.
