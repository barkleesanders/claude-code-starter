# Stitch Workflows — Cloud Design Generation via Stitch MCP

**Load this when**: the user wants Stitch to generate/edit screens, manage Stitch projects, or extract a design system from existing Stitch screens.

**Assumes**: `aesthetic-core.md` already ran and a direction is locked. Step 3 below (atmosphere) reuses that direction rather than re-choosing.
...
Use the vague→professional table from `aesthetic-core.md`. Don't restate it; pull from there.

### Step 3: Atmosphere — reuse direction from `aesthetic-core.md`
**Do NOT pick a new vibe here.** The direction was already locked in Phase 0 (brutalist / luxury / retro-futuristic / etc.). This step's job is just to translate that direction into Stitch-friendly language.

Translation hints by direction:

| Direction (Phase 0) | Stitch-friendly description |
|---|---|
| Brutalist / raw | "Exposed grid, oversized condensed sans, raw concrete palette, zero shadows, hard rule lines" |
| Luxury / refined | "Generous whitespace, fine hairlines, deep black + champagne accent, serif display + clean sans body" |
| Retro-futuristic | "CRT-glow accents, scanline overlay, chromatic aberration on the hero, mono + display pairing" |
| Editorial / magazine | "Big drop cap, asymmetric grid, body-text-as-hero, footnote-style decorative numbering" |
| Maximalist chaos | "Overlapping layers, clashing-on-purpose accent against dominant base, vibrating motion on hero" |
| Playful / toy-like | "Bouncy spring motion, candy palette, fully-rounded corners, oversized illustration" |
| Organic / natural | "Hand-drawn curves, paper grain background, muted earth palette, gentle scroll-revealed motion" |

If the direction isn't in this table, write the equivalent description yourself — but it MUST be the same direction picked in Phase 0.

### Step 4: Structure the final prompt

```
[Direction-driven mood line from Step 3, plus page purpose]
...
1. **Enhance prompt** using pipeline above (direction comes from Phase 0)
...
[Direction from Phase 0 + aesthetic philosophy]
...
- **Reuse Phase 0 direction**: never re-choose the vibe inside Stitch prompts — it was already locked
