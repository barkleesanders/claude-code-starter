...
## Keep Debugging Always On

The "Allow debugging" modal fires **once per tab** when a daemon first connects. To minimize popups:
...
2. **Daemons auto-extend**: Any command resets the 20-minute idle timer. Active debugging sessions stay warm indefinitely.

3. **Other skills auto-warm**: `/carmack` and `/ship` now auto-warm CDP daemons for production URLs before doing live checks.
