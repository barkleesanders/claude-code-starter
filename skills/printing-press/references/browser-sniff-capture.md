...
```
If auto-connect fails (no debug port), explain: "fcdp's bridge isn't answering. Run `launchctl kickstart -k gui/$(id -u)/com.barklee.fcdp-bridge` (one-time: load ~/tools/fcdp/extension via chrome://extensions), or pick option 2."
