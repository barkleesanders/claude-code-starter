...
## Hard Rule: Prod social OAuth needs its OWN client_id/secret (MANDATORY)

**Clerk PRODUCTION instances do NOT inherit the shared dev OAuth client.** If `connection_oauth_<provider>.client_id` is empty while `enabled=true`, "Continue with <Provider>" lands on the provider's `/signin/oauth/error?authError=...&client_id=` page (decoded: `invalid_request`, `Missing required parameter: client_id`).

**Fix paths:**
- **Disable on prod:** `clerk api -X PATCH /v1/instance/social_connections/oauth_<provider> --data '{"enabled":false}' --instance prod`
- **Configure a real client:** create a Google/GitHub/etc. OAuth client at the provider, then:
  ```bash
  clerk api -X PATCH /v1/instance/social_connections/oauth_google \
    --data '{"client_id":"<id>","client_secret":"<secret>","enabled":true}' \
    --instance prod --dry-run
  ```
  For Google: authorized origins = `https://<root>` + `https://clerk.<root>`; redirect URI = `https://clerk.<root>/v1/oauth_callback`.

**Why:** 2026-05-09 incident — prod SSO silently broke for every social provider after enabling them via dashboard without supplying clientId. See reference incident in `~/.claude/projects/-Users-<you>/memory/reference_clerk_prod_oauth_setup.md` and beads HOME-b793.
...
- Resolve the target user/org/session against live Clerk before any mutation; confirm the subject; prefer read/dry-run first.
